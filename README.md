...
This repository includes Dockerfiles for the backend and frontend plus a `docker-compose.yml` to run Postgres, the NestJS backend, and the React frontend locally.

- Backend Dockerfile: `daleel-backend/Dockerfile`
- Frontend Dockerfile: `daleel-frontend/daleel-master/Dockerfile`
...
Notes:
- The backend expects `DATABASE_URL`, `SUPABASE_URL`, and `SUPABASE_SERVICE_ROLE_KEY` environment variables.
- The compose file sets `DATABASE_URL` for the included Postgres container.
- For local dev without Supabase, you can use placeholder values; the app will fail on Supabase-dependent routes but will start.
- Backend runs on port `3001`. Frontend is served on port `80`.
- Frontend can be accessed at `http://localhost` after the frontend container is running.

AWS deployment notes:
- For a simple container deployment consider pushing the images to Amazon ECR and using ECS (Fargate) or Elastic Beanstalk.
- For a Kubernetes deployment use the existing `k8s/` manifests and consider deploying to EKS; Terraform files in `Terraform/` can be adapted for infrastructure provisioning.

If you want, I can:
- build and test the images locally now, or
- add ECR/ECS CI steps and example Terraform for ECR/ECS/EKS deployment.

### GitHub Actions: ECR / ECS

I added a workflow to build and push images to Amazon ECR and deploy to ECS.

Required GitHub repository secrets:
- `AWS_ACCESS_KEY_ID` – IAM key with permissions to push to ECR and deploy ECS.
- `AWS_SECRET_ACCESS_KEY` – corresponding secret.
- `AWS_REGION` – AWS region (e.g., `us-east-1`).
- `AWS_ACCOUNT_ID` – your AWS account id.
- `ECS_CLUSTER` – the ECS cluster name to update.
- `ECS_BACKEND_SERVICE` – ECS service name for the backend.
- `ECS_FRONTEND_SERVICE` – ECS service name for the frontend.

Workflow path: `.github/workflows/aws-ecr-ecs.yml`
Task definition templates: `deployment/ecs/backend-task-def.json`, `deployment/ecs/frontend-task-def.json` (they include placeholder ARNs for `executionRoleArn` and `taskRoleArn` — replace with your roles or set them in the templates before deploying).

If you want, I can:
- fill in a sample Terraform module to create ECR repos, IAM roles, and ECS services, or
- create a GitHub Actions variant that deploys to EKS using the existing `k8s/` manifests.
