...
import { exportInvoicesToCSV } from '../../utils/csvExportUtils';
...
function BusinessDetail() {
  ...
  const handleExportToCSV = () => {
    const invoices = getInvoicesByQuarter(rightView);
    const quarterLabel = rightView === 'q1' ? 'Q1' :
                        rightView === 'q2' ? 'Q2' :
                        rightView === 'q3' ? 'Q3' : 'Q4';
    const filename = `${business.name}_${quarterLabel}_invoices_${new Date().toISOString().split('T')[0]}.csv`;
              ...
              <div className="invoices-list-box">
                <div className="business-info-header">
                  ...
                  {getInvoicesByQuarter(rightView).length > 0 && (
                    <button className="export-csv-btn" onClick={handleExportToCSV}>
                      📥 Export to CSV
                    </button>
                  )}
                </div>
